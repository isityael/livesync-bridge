        ...
        if (old && Math.abs(this.compareDate(info, old)) < 3600) {
            ...
            if (oldDoc && "data" in oldDoc) {
                const d = oldDoc.type == "plain"
                    ...
                    : createBinaryBlob(
                        new Uint8Array(decodeBinary(oldDoc.data)),
                    );
    ...
    async start(): Promise<void> {
        ...
        if (w && "tweak_values" in w) {
            if (this.config.useRemoteTweaks) {
                ...
                this.config.customChunkSize = tweaks.customChunkSize ??
                    this.config.customChunkSize;
                this.config.minimumChunkSize = tweaks.minimumChunkSize ??
                ...
                if (
                    tweaks.usePathObfuscation &&
                    !this.config.obfuscatePassphrase
                ) {
                    throw new Error(
                ...
                this.config.maxAgeInEden = tweaks.maxAgeInEden ??
                    this.config.maxAgeInEden;
                this.config.maxTotalLengthInEden =
                    tweaks.maxTotalLengthInEden ??
                        this.config.maxTotalLengthInEden;
                this.config.maxChunksInEden = tweaks.maxChunksInEden ??
                    this.config.maxChunksInEden;
                this.config.useEden = tweaks.useEden ?? this.config.useEden;
                if (
                    !this.config.enableCompression != !tweaks.enableCompression
                ) {
                ...
                this.config.enableChunkSplitterV2 =
                    tweaks.enableChunkSplitterV2 ??
                        this.config.enableChunkSplitterV2;
                this.config.chunkSplitterVersion =
                    tweaks.chunkSplitterVersion ??
                        this.config.chunkSplitterVersion;
                this.config.E2EEAlgorithm = tweaks.E2EEAlgorithm ??
